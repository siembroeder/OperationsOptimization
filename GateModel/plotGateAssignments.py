...
def plot_timetable_broken(x_solution, comp_ir, p_ij, e_i, f_i, all_aircraft, gate_coords, dom_gates, int_gates, all_times, distinct_times, dom_aircraft, int_aircraft, 
    ...
    apron_spacing = 1.05
    apron_assignments = {}  # {aircraft: level}
    ...
    for ac in all_aircraft:
        ...
        occupied_levels = set()
        for other_ac, other_level in apron_assignments.items():
            other_intervals = comp_ir[other_ac]
            other_occupied = [r for r, occupied in enumerate(other_intervals) if occupied]
            # Check if there's any overlap
            if any(r in other_occupied for r in occupied_intervals):
        ...
        while level in occupied_levels:
            level += 1
        apron_assignments[ac] = level
    
    max_apron_level = max(apron_assignments.values(), default=-1)
    ...
    for ac in all_aircraft:
        ...
        if gate == apron:
            y = apron_base_y + apron_assignments[ac] * apron_spacing
        else:
            y = gate_y[gate]
